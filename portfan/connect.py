"""Native cognis-connect emit for portfan — forward findings to any platform.

Maps portfan's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    portfan ... --format json | portfan-emit --to stix
    portfan-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('service', '') + ' on ' + rec.get('host', '')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'vulnerability')
